gform.stencils.admin_dashboard = `<div class="row">
    <div class="col-md-9">  
    <div class="pull-right" style="width: 300px;height: 40px;" id="search"></div>
      ...
      <ul class="nav nav-tabs" role="tablist">
        ...
        {{#workflow_developers.length}}<li role="presentation"><a href="#workflows" aria-controls="workflows" role="tab" data-toggle="tab"><i class="fa fa-check"></i> <span class="hidden-xs hidden-sm">My Workflows</span></a></li>{{/workflow_developers.length}}
      ...
      <div class="tab-content">
        ...
        <div class="filterable grid-item" style="display:inline-block;">
            ...
            <div class="" style="margin-top:5px;font-weight:400;font-size:large;color:#777;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">{{name}}</div>
        ...
        <div role="tabpanel" class="tab-pane groups {{^app_developers.length}}{{^workflow_developers.length}}active{{/workflow_developers.length}}{{/app_developers.length}}" id="groups">
        <div class="masonry-grid">
        {{#group_admins}}
        <div class="filterable grid-item" style="display:inline-block;">
            ...
            <div style="margin:10px 0;/*border-top:solid 1px #ddd;border-bottom:solid 1px #fff*/"></div>
              {{#group}}
                    ...
                    {{/members_count}}
										...
										{{/appinstances_count}}
                    {{#workflowinstances_count}}
										<a href="/admin/groups/{{id}}/workflowinstances" class="list-group-item" style="border-left:solid #31708f">
											<i class="fa fa-list text-info"></i> <span class="text-info">Workflow Instances</span> <span class="badge">{{workflowinstances_count.aggregate}}</span>
										</a>
										{{/workflowinstances_count}}
										{{#endpoints_count}}
             ...
             <div class="group-info">
                ...
                {{/appinstances_count}}
                {{^workflowinstances_count}}
                <a href="/admin/groups/{{id}}/workflowinstances" class="text-default" style="padding:5px;font-size:20px;">
                  <span class="fa fa-list" title="Workflows" data-toggle="tooltip" data-placement="top"></span>
                </a>
								{{/workflowinstances_count}}
                {{^endpoints_count}}
        ...
        <div role="tabpanel" class="tab-pane workflows" id="workflows">
        <div class="masonry-grid">
        {{#workflow_developers}}
        {{#workflow}}

        <div class="filterable grid-item" style="display:inline-block;">
        		<div class="panel panel-default" style="border-top: solid 3px #9ac337;">
              <div class="avatar" title="{{user.first_name}} {{user.last_name}}" data-toggle="tooltip" data-placement="top" style="background:{{user.unknown}}">{{user.initials}}</div>
              <a href="/admin/workflows/{{id}}" class="parent-hover pull-right" style="margin:5px 10px;color:#aaa"><i class="fa fa-pencil"></i></a>

            <div class="" style="margin-top:5px;font-weight:400;font-size:large;color:#777;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">{{name}}</div>
              <div class="label" style="background:#8bb4da; font-weight:normal"><i class="fa fa-clock-o"></i> Updated: <time class="timeago" datetime="{{updated_at}}" title="{{updated_at}}"></time></div>
                ...
                <p style="color: #666;font-size: small;">{{description}}</p>
                  ...
                  <div class="workflowInstance">
                    ...
                    <div class="dropdown parent-hover pull-right">
                      ...
                      <ul class="dropdown-menu" aria-labelledby="dLabel">
                        <li><a target="_blank" href="/workflow/{{group_id}}/{{slug}}"><i class="fa fa-external-link"></i> Visit</a></li>
                        <li><a target="_blank" href="/admin/workflowinstances/{{id}}/report"><i class="fa fa-list-ol"></i> Report</a></li>
                        
                        <li><a href="/admin/workflowinstances/{{id}}"><i class="fa fa-pencil"></i> Edit</a></li>
                      </ul>
                    </div>
                    ...
                    <a class="collapsed" role="button" data-toggle="collapse" href="#collapseExample{{id}}" aria-expanded="false" aria-controls="collapseExample{{id}}"><span class="fa fa-collapse"></span><span class="pull-right text-muted">{{group.name}}</span> <div> <i class="fa {{#app_version_id}}fa-lock{{/app_version_id}} {{^app_version_id}}fa-lock-{{app_version_id}}{{/app_version_id}}"></i> {{name}}</div>
                      ...
                      <i title="Suppress Emails" data-toggle="tooltip" data-placement="top"  class="fa fa-envelope text-danger"></i>
                      {{/configuration.suppress_emails}}
                      {{^configuration.suppress_emails}}
                      <i title="Send Emails" data-toggle="tooltip" data-placement="top"  class="fa fa-envelope text-success"></i>
                      {{/configuration.suppress_emails}}
                    ...
                    <div class="collapse" id="collapseExample{{id}}">
                      ...
                      <h5>{{version.summary}}</h5>
                      <p class="text-muted">{{version.description}}</p>
                      <dl class="dl-horizontal"><dt>Initial State:</dt><dd>{{configuration.initial}}{{^configuration.initial}}<span class="text-danger"> - None - </span>{{/configuration.initial}}</dd></dl>
