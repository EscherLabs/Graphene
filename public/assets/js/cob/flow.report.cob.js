          ...
          $.ajax({
            ...
            success: function (resources, response) {
              ...
              mappedData.history = _.map(data, function (event) {
                var newEvent = _.pick(event, 'user', 'deleted_by', 'id', 'data', 'comment', 'action', 'signature', 'status', 'created_at', 'updated_at', 'opened_at', 'file', 'log', 'mime_type', 'path', 'name', 'icon', 'preview', 'date', 'deleted_at', 'deleted_by');
              ...
              $('.row .list').on('click', '.filterable.submission', function (e) {
                ...
                if (e.currentTarget.dataset.current) {
                  ...
                  if (mappedData.workflow.instance.version.code.form.files && mappedData.current_state.uploads) {
                    if ($("#uploader_" + mappedData.original.workflow_submission_id).length) {
                      $("#uploader_" + mappedData.original.workflow_submission_id).html('');
                      fileLoader = new Dropzone("#uploader_" + mappedData.original.workflow_submission_id, {
                        timeout: 60000, url: "/api/workflowsubmissions/" + mappedData.original.workflow_submission_id + "/files", init: function () {
                  ...
                  $('.workflow-files, .report').on('click', '[data-id]', function (e) {
                    if (e.currentTarget.dataset.action == 'delete') {
                      ...
                      e.preventDefault();
                      myModal = new gform({
                        legend: "Edit file name", data: _.find(mappedData.history, { id: parseInt(e.currentTarget.dataset.id) }), fields: [
