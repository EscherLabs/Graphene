...
gform.types['user']= _.extend({}, gform.types['smallcombo'], {
  defaults:{label:'User <span class="text-success pull-right">{{value}}</span>',search:"/api/users/search/{{search}}{{value}}",format:{label:"{{first_name}} {{last_name}}",value:"{{email}}", display:"{{first_name}} {{last_name}}<div>{{email}}</div>"}}
})
