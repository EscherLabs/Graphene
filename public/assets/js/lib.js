...
$g = function (options) {
  ...
  let api = {
    ...
    confirm: (message, action) => {
      $g.modal({
        title: 'Confirm', content: message, actions: [
...
$g.on('loaded debug', (e) => {
  ...
  switch (resource_type) {
    case 'app':
      obj = {
        resources: _.reduce(debug.app.config.resources, (result, resource) => {
...
gform.types['files'] = _.extend({}, gform.types['combobox'], {
  ...
  defaults: {
    options: 'files', strict: true, custom: {
      name: "addFile", display: '<div style="line-height:50px"><span class="label label-success">Upload New File</span> <span class="pull-right"><i class="fa fa-upload"></i></span></div>', action: function (e) {
...
gform.types['currency'] = _.extend({}, gform.types['input'], {
  defaults: { placeholder: "$0.00", value: "", validate: [{ type: 'numeric' }/*,{type:'numeric'}*/] },
  toString: function (name, report) {
  ...
  set: function (value) {
    if (!('el' in this)) { this.internalValue = value; return; };
...
_.each(["success", "danger", "info", "warning", "default", "primary", "link"], actionType => {
  gform.types[actionType] = _.defaultsDeep({ toString: () => '' }, gform.types['button'], {
    defaults: {
      "action": "save",
