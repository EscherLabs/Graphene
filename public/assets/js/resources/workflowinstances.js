...
$g.getData(
  [
    ...
    "/api/workflows/group/" + resource_id,
  ],
  (workflowinstances, groups, icons, workflows) => {
    grid = new GrapheneDataGrid({
        ...
        resource_id !== ""
          ? [
              ...
              },
              {
                ...
                max: 1,
                min: 1,
                label: '<i class="fa fa-list-ol"></i> Report',
              },
              {
                name: "raw_data",
                max: 1,
                min: 1,
                label: '<i class="fa fa-table"></i> Raw Data',
              },
              "|",
              { name: "create" },
      ...
      .on("model:report", e => {
        document.location =
          "/admin/workflowinstances/" + e.model.attributes.id + "/report";
      })
      .on("model:raw_data", e => {
        document.location =
          "/admin/workflowinstances/" + e.model.attributes.id + "/raw";
      })
      .on("sort", e => {
        var tempdata = _.map(
          e.grid.models,
