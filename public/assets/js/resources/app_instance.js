      ...
      success: function (versions) {
        ...
        $("#find").on("click", function () {
          $.get(root + "/pages", pages => {
            if (pages.length > 0) {
              modal({
                title: "This App Instance was found on the following pages",
              ...
              modal({
                ...
                content:
                  "This App Instance is not currently placed on any pages.",
              });
            }
          });
        });
        ...
        $("#save").on("click", function () {
          ...
          if (typeof $g.forms.options !== "undefined") {
            if (!$g.forms.options.validate()) {
              $g.alert({ status: "error", content: "Check Options" });
              return;
            }
          }
          ...
          $.ajax({
            ...
            data: JSON.stringify({
              ...$g.forms.main.get(),
              options:
                typeof $g.forms.options !== "undefined"
                  ? $g.forms.options.get()
                  : {},
              user_options_default:
                typeof $g.forms.user_options_default !== "undefined"
                  ? $g.forms.user_options_default.get()
                  : {},
              resources:
                typeof $g.forms.resources !== "undefined"
                  ? $g.forms.resources.get().resources
                  : [],
            }),
            success: () => {
        ...
        if (forms.find(form => form.name == "Options").content) {
          new gform(
            {
              ...JSON.parse(forms.find(form => form.name == "Options").content),
              name: "options",
        ...
        if (forms.find(form => form.name == "User Options").content) {
          new gform(
            {
              ...JSON.parse(
                forms.find(form => form.name == "User Options").content
              ),
          ...
          $("#useroptionstab").toggle(
            !!$g.forms.user_options_default.fields.length
          );
        }
