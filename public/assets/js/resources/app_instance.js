...
$g.getData([root, '/assets/data/icons.json', '/api/groups/' + group.id + '/endpoints'], (appinstance, icons, endpoints) => {
	...
	$.ajax({
		...
		success: function (versions) {
			...
			if (forms.find(form => form.name == "User Options").content) {
				new gform({
					...JSON.parse(forms.find(form => form.name == "User Options").content),
