...
$.ajax({
	...
	success: function(data){
		tableConfig.schema = [
			...
			{label: 'API Version', name:'api_version_id', show: false,type:'select',options:'/api/proxy/'+slug+'/api_versions',
			template:"{{summary}}{{^summary}}{{description}}{{^description}}-{{/description}}{{/summary}}",
			format:{label:"{{summary}}{{^summary}}{{description}}{{^description}}-{{/description}}{{/summary}}",value:'{{id}}'}},			
