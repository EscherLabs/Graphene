myconditions = [
	{
		label: false, columns: 12, name: 'op', type: "switch", format: { label: '{{label}}' }, options: [{ label: "or", value: 'or' }, { label: "and", value: 'and' }], value: 'and', show: [{
			type: "test", name: "op", test: function (field, args) {
				return true;
			}
		}]
	},
...
displayFields = _.map([
	...
	{
		type: 'select', label: 'Offset', value: 0, name: 'offset', min: 0, max: 11, format: {
			label: "{{value}} Column(s)", value: function (e) {
				return parseInt(e.value);
			}
...
Cobler.types.bool = function (container) {
	...
	var fields = [
		...
		type: 'fieldset', label: false, array: { min: 2, max: 2 }, columns: 12, name: 'options', fields: [
			{ title: '{{#parent.index}}True{{/parent.index}}{{^parent.index}}False{{/parent.index}} Label', 'name': 'label', parse: [{ type: "requires" }] },
			{
				title: '{{#parent.index}}True{{/parent.index}}{{^parent.index}}False{{/parent.index}} Value', 'name': 'value', parse: [{ type: "requires" }], value: function (e) {
					return (typeof e.initial.value !== 'string') ? '' : e.initial.value;
				}
			},
		]
	}], function (item) {
...
Cobler.types.section = function (container) {
	...
	var fields = [
		...
		{
			target: "#display", columns: 9, type: "button", modifiers: "btn btn-default pull-left margin-bottom", label: "Manage Section", action: "manage", name: "manage", show: [{
				type: "test", name: "manage", test: function (e) {
