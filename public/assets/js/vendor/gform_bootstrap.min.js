        ...
        array: {
            template: (data) => {
              ...
              <div data-id="{{id}}" class="col-xs-12">
                ...
                {{#array}}{{#append.enable}}<button data-ref="{{id}}" class="create btn btn-success gform-append">{{append.label}}{{^append.label}}Click here to create first {{label}}{{/append.label}}</button>{{/append.enable}}{{/array}}
                </div>`, data || this)
        ...
        return (options, field) => {
            ...
            if (typeof field.item.value === 'function' || (typeof field.item.method === 'string' && typeof field.owner.methods[field.item.method] == 'function')) {
                field.derivedValue = (e) => {
                    let localVal = (field.owner.methods[field.item.method] || field.item.value).call(null, { form: field.owner, field: field, initial: field, ...e });
            ...
            if (field.fillable) {
                ...
                if (typeof field.derivedValue == 'function') {
                    ...
                    field.value = field.derivedValue({ initial: field, field: field, form: field.owner });
...
<form id="{{name}}" {{^autocomplete}}autocomplete="false"{{/autocomplete}} name="{{name}}" class="gform tab-content {{#horizontal}} smart-form-horizontal form-horizontal{{/horizontal}} {{modifiers}}" {{#action}}action="{{action}}"{{/action}} onsubmit="return false;" {{#method}}method="{{method}}"{{/method}}>{{^legendTarget}}{{#legend}}<legend>{{{legend}}}</legend>{{/legend}}{{/legendTarget}}    
	...
	<div style="display:block" class=" modal gform {{modifiers}} {{#horizontal}} form-horizontal{{/horizontal}}" id="myModal" name="modal_{{name}}" data-update="{{update}}" data-append="{{append}}" {{#focus}}tabindex="-1"{{/focus}} role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
