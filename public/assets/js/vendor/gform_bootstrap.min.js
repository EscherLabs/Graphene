...
gform.ajax = function (options) {
    ...
    if (options.sourceID) {
        var current = _.find(gform.xhrPool, { id: (options.sourceID + options.path) })
        if (current) {
            current.abort();
            gform.xhrPool = _.filter(gform.xhrPool, x => x != current);
        }
        request.id = options.sourceID + options.path
...
gform._subscribeByName = function (conditions, callback) {
    ...
    for (var i in conditions) {
        if (typeof conditions[i].conditions == 'object') {
            ...
            // var temp = this.parent.items.find((conditions[i].name || this.name));
...
gform._lookup = (field, args, _lookup) => {
    if (field.name == args.name || typeof args.name == 'undefined' || !args.name) { return field; }
    ...
    while (!looker && typeof searchField.parent !== 'undefined') {// && !(searchField instanceof gform)) {
        ...
        var matches = searchField.items.filter({ name: args.name }, args.depth || 10);

        if (matches.length > 0) {
            looker = _.sortBy(matches, 'parsable')[0];
        }
    }
...
gform.conditions = {
    ...
    requires: (field, args, _lookup = _lookup) => {
        let targetField = _lookup(field, args, _lookup);
...
gform.types['smallcombo'] = _.extend({}, gform.types['input'], {
    ...
    set: function (value, silent, input) {
        ...
        if (typeof item !== 'undefined') {
            if (!input || (input == 'nosearch' && this.el.querySelector('[type=smallcombo]') !== document.activeElement)) {
            ...
            if (typeof this.search == 'string' && input !== 'nosearch') {
                gform.ajax({
                    sourceID: this.id,
                    path: gform.renderString(this.search, { value: value }), success: function (value, data) {
                        if (data.length > 0) {
                            if (!this.active) return;
                            ...
                            this.menu.innerHTML = "";
                            var item = _.find(this.options, { value: value }) || _.find(this.options, { label: value })
                            this.shown = (typeof item == 'undefined' && data.length > 1);
                            this.menu.style.display = (this.shown) ? 'block' : 'none';

                            this.value = (item || { value: value }).value;

                            index = this.options.length;
                            this.options = this.mapOptions.getoptions();

                            this.menu.innerHTML = "";
                            this.options = this.options.concat(_.map(data, this.createLI))
    ...
    initialize: function () {
        ...
        this.processOptions = function (option) {
            ...
            if (typeof option.optgroup !== 'undefined') {
                ...
                option = this.createLI(option)
            }
        }
        this.createLI = function (option) {
            ...
            if (this.filter !== false && (this.combo.innerText == "" || _.score(option.label.toLowerCase(), this.combo.innerText.toLowerCase()) > .6)) {
                var li = document.createElement("li");
        ...
        this.renderMenu = function () {
            ...
            if (typeof this.search == 'string') {
                gform.ajax({
                    sourceID: this.id,
                    path: gform.renderString(this.search, { search: this.combo.innerText }), success: function (data) {
                        index = this.options.length;
                        this.options = this.options.concat(_.map(data, this.createLI))
