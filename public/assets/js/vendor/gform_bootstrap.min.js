...
gform._lookup = (field, args, _lookup) => {
    ...
    searchField.items.filter({ name: args.name.split('.')[0] }, args.depth || 10)[0].items.filter({ name: args.name.split('.')[1] }, args.depth || 10)[0]
    ...
    if (args.name.indexOf('/')) {
        ...
        if (looker && searchObj.length > 0) {
            _.each(searchObj, (looker, search) => {
                if ('items' in looker) {
                    looker.items.filter({ name: search }, args.depth || 10);
                }
                return looker;
            }, looker)
        }
