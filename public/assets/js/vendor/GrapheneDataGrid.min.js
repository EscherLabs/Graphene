...
gform.stencils.queryStyles = `<style>#tags{display: flex;
  font-weight:bold;
  gap:.75em;
  flex-wrap: wrap;
  margin-bottom:10px;
...
{{#options.query}}<input type="text" autocomplete="off" name="query" class="form-control pull-left" style=" margin-bottom:10px;max-width:{{options.query}}%;" placeholder="deleted:false">{{/options.query}}
  ...
  applyFilter: function (model, options, filter) {
    ...
    mapfunc = search => {
      ...
      switch (search.action) {
        ...
        case "<>": //number range
          let [min, max] = _.map(search.raw.split(" - "), i =>
            parseInt(_.toString(_.trim(i, ["[", "]", '"', "'"])))
          );

          let num = parseInt(finding);
          return (
            ((isNaN(min) || num > min) && (isNaN(max) || num < max)) ||
            (search.format && (num == min || num == max))
          );
        case "@": //date range
          let temp = search.raw.split(" - ");

          let [from, to] = _.map(temp, i =>
            Date.parse(_.toString(_.trim(i, ["[", "]", '"', "'"])))
          );

          let target = Date.parse(finding);
          return (
            ((isNaN(from) || target > from) && (isNaN(to) || target < to)) ||
            (search.format && (target == to || target == from))
          );
        default:
