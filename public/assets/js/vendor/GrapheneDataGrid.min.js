GrapheneDataGrid = function (options) {
	...
	var summary = {
		'[[': '{{', ']]': '}}', checked_count: 0, 'items': _.map(options.filterFields, function (val) {
	...
	function onload($el) {
		...
		if ($el.find('.filter').length) {
			this.filter = new gform({
				actions: [], collections: this.collections, name: 'filter' + this.options.id, clear: false, fields: options.filterFields, default: {
					target: function () {
						return '[name="' + this.name + '"],[data-inline="' + this.name + '"]'
					}, hideLabel: true, type: 'text', format: { label: '{{label}}', value: '{{value}}' }
				}
			}, $el.find('.filter')[0]).on('input', function () {
		...
		ordered = _.filter(ordered, function (anyModel) {
			...
			for (var filter in options.search) {
				...
				if (_.filter(options.filterFields, { id: filter })[0] && typeof _.filter(options.filterFields, { id: filter })[0].options == 'undefined') {
					...
					temp = (anyModel.display[this.filterMap[filter]] + '' == options.search[filter] + '') ||
						(anyModel.attributes[this.filterMap[filter]] + '' == options.search[filter] + '') ||
						(typeof anyModel.attributes[this.filterMap[filter]] == "object" && anyModel.attributes[this.filterMap[filter]] !== null && (anyModel.attributes[this.filterMap[filter]].indexOf(options.search[filter]) != -1 ||
							anyModel.attributes[this.filterMap[filter]].indexOf(options.search[filter] + "") != -1)
						)
				}
				keep = temp;
