...
';
        ...
        // 01/22/2021, AKT - Fixed the code below to prevent sending emails to the owner:
        // if the actor is internal, if the owner is in the assigned group
        if ((isset($state_data['assignment']['user']) && $state_data['assignment']['user']['unique_id'] !== $state_data['owner']['unique_id']&& isset($state_data['actor']['unique_id']))
            || (isset($state_data['assignment']['group']) &&
                sizeof(array_filter($state_data['assignment']['group']['members'], // Do not send update emails if owner is in the assigned group
                    function($el) use($state_data){ return $el['unique_id']===$state_data['owner']['unique_id'];}))===0
                && isset($state_data['actor']['unique_id']))) {
            $to = $state_data['owner'];
