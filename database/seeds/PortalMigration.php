        ...
        try {
            ...
            foreach($apps_db as $index => $app_db) {
                try {
                    ...
                    $app->site_id = $site_id;
                    $app->description = isset($groups_index[$app_db->group_id])?'Group: '.$groups_index[$app_db->group_id]->name:'';
                    $app->tags = isset($groups_index[$app_db->group_id])?'group:'.strtolower($groups_index[$app_db->group_id]->slug):''; 
                    ...
                    $app_db->template = str_replace('pageID','page_id',$app_db->template);  
                    $app_db->script = str_replace('pageID','page_id',$app_db->script);  
                        ...
                        foreach($page_column_db as $page_widget_db) {
                            ...
                            if ($page_widget_db->widgetType=='LinkCollection') {
                                ...
                                $page_content_array['sections'][$page_column_num][] = [
                                    ...
                                    'text' => $text,
                                    'device' => isset($page_widget_db->device)?$page_widget_db->device:'',
                                    'enable_min' => isset($page_widget_db->enable_min)?$page_widget_db->enable_min:false,
                                    'collapsed' => isset($page_widget_db->collapsed)?$page_widget_db->collapsed:false,
                                    'container' => isset($page_widget_db->container)?$page_widget_db->container:true,
                                    'titlebar' => isset($page_widget_db->container)?$page_widget_db->container:true,
                                    'guid' => isset($page_widget_db->guid)?$page_widget_db->guid:str_random(32),
                                    'limit' => $limit_boolean,
                                    'group' => $limit_groups,
                                    'editor' => $editor,
